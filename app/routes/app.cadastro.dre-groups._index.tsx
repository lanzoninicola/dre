...
function CompanyCard({ company }: { company: CompanyWithStats }) {
  return (
    <Link
      to={`/app/cadastro/dre-groups/${company.id}`}
...
function CompanyListItem({ company }: { company: CompanyWithStats }) {
  return (
    <Link
      to={`/app/cadastro/dre-groups/${company.id}/receita`}
...
export default function AccountPlanIndex() {
  ...
  const viewOptions = [
    { value: "grid" as const, label: "Grade", icon: Grid3X3, shortLabel: "Grid" },
    { value: "list" as const, label: "Lista", icon: List, shortLabel: "List" }
  ];
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4">
        ...
        </div> {/* ← ESTE fechamento estava faltando */}
        ...
        <div className="max-w-7xl mx-auto px-4 mb-6">
          <div className="max-w-2xl mx-auto">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <FilterToggle value={viewMode} onChange={setViewMode} options={viewOptions} />
            </div>
          </div>
        </div>
          ...
          <div className="mb-6">
            <p className="text-sm text-gray-600">
              {filteredCompanies.length > 0
                ? `Encontradas ${filteredCompanies.length} empresa${filteredCompanies.length !== 1 ? "s" : ""
                } para "${searchTerm}"`
